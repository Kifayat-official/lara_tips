...
if($sdiv_cd>0 && $lat>0 && $lng>0){
		...
		try { 
		 ...
		 $fdr_query = "SELECT FEEDER_CODE,FDRNAME,SDIVCODE, SDIVNAME,LATITUDE,LONGITUDE,dist as distance from (SELECT FEEDER_CODE,FDRNAME,SDIVCODE, SDIVNAME,LATITUDE,LONGITUDE,round(distance,2) as dist FROM ( SELECT *, ( ( ( acos( sin(('$lat' * pi() / 180)) * sin(( `LATITUDE` * pi() / 180)) + cos(('$lat' * pi() /180 )) * cos(( `LATITUDE` * pi() / 180)) * cos((('$lng' - `LONGITUDE`) * pi()/180))) ) * 180/pi() ) * 60 * 1.1515 * 1.609344 ) as distance FROM `vw_consumers_xy`) vw_consumers_xy WHERE distance <100 ORDER BY distance ) vw_consumers_xy group by feeder_code ORDER BY distance";	//radius 100. get 10 most nearest records
