...
<div class="box">
    <div class="box-header with-border">
        <div class="row">
            <div class="col-md-10">
                <h3 style="margin: 0;">
                    Test Session ID: PITC/INTR/LB/{{ str_pad( $mdc_test_session->id_numeric ,6,"0",STR_PAD_LEFT) }}
                    - <small>(Company: {{$mdc_test_session->company->name}})</small>
                </h3>
            </div>
            <div class="col-md-2">
                <button id="btn-run-all-tests" onclick="configureAllTests()" class="btn btn-success pull-right" {{ $mdc_test_session->is_finished == 1 ? 'disabled' : '' }}>
    ...
    <!-- /.box-header -->
    <div class="box-body">
        @foreach(\App\TestGroup::get() as $test_group)
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">{{$test_group->name}}</h3>
            </div>
                ...
                <div class="row tests-container">
                        ...
                        <div class="box box-success ">
                            ...
                            <div class="box-body">
                                <div class="row">
                                    ...
                                    @if(isInitializationTest($test))
                                    <?php $test_number++;?>
                                    @component('tests.components.test_button', [
                                    'test' => $test,
                    ...
                    <div class="col-md-4">
                        <div class="box box-success ">
                            ...
                            <div class="box-body">
                                @foreach($tests as $test)
                                @if($test->testType->idt == 'read' && $test->test_group_id == $test_group->id && !isInitializationTest($test))
                                <?php $test_number++;?>
                                @component('tests.components.test_button', ['test' => $test, 'mdc_test_session' => $mdc_test_session, 'test_number' => $test_number])
                                @endcomponent
                                @endif
                                @endforeach
                            </div>
                    ...
                    <div class="col-md-4">
                        <div class="box box-success col-md-6">
                            ...
                            <div class="box-body">
                                @foreach($tests as $test)
                                @if($test->testType->idt == 'write' && $test->test_group_id == $test_group->id && !isInitializationTest($test))
                                <?php $test_number++;?>
                                @component('tests.components.test_button', ['test' => $test, 'mdc_test_session' => $mdc_test_session, 'test_number' => $test_number])
                                @endcomponent
                                @endif
                    ...
                    <div class="col-md-4">
                        <div class="box box-success ">
                            ...
                            <div class="box-body">
                                @foreach($tests as $test)
                                @if($test->testType->idt == 'on_demand' && $test->test_group_id == $test_group->id && !isInitializationTest($test))
                                <?php $test_number++;?>
                                @component('tests.components.test_button', ['test' => $test, 'mdc_test_session' => $mdc_test_session, 'test_number' => $test_number])
                                @endcomponent
                                @endif
            ...
            <div class="box-footer text-right">
                ...
                @if( \Auth::user()->hasPermission('finish_test') )
                <button onClick="finishTestSession('{{$mdc_test_session->id}}')" class="btn btn-sm btn-primary" {{!$mdc_test_session->isAllTestsExecuted() || $mdc_test_session->is_finished == 1 ? 'disabled' : ''}}>
                    <i class="fa fa-save"></i>
                    Finish Test
                </button>
                @endif
                @if($mdc_test_session->is_finished)
                Test Finished
                @endif
            </div>
...
<div class="modal fade" id="configureAllTests">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @component('tests.components.select_meters', [
                    'mdc_test_session' => $mdc_test_session    
                ])
                @endcomponent
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                <button onClick="runAllTests()" type="button" class="btn btn-success" {{ $mdc_test_session->is_finished == 1 ? 'disabled' : '' }}>
                    <i class="fa fa-play-circle"></i> Run All Tests
                </button>
            </div>
        </div>
...
<script>
    ...
    function finishTestSession(testSessionId) {
        $.ajax({
            type: 'post',
            url: "{{url('finish_test_session')}}",
            data: {id: testSessionId, _token: "{{@csrf_token()}}"}
        })
        .done(function(data) {
            location.reload();
        })
    ...
    $(document).ready(function() {
        ...
        $('.btn-test').click(function() {
            ...
            loadTestView("{{$mdc_test_session->id}}", testId, testName);
        });
